...
void multiply(const std::vector<double> &m1, const std::vector<double> &m2,
...
int main(int argc, char **argv) {
  ...
  // lambda function: a local function that captures [] something, receives
  // something (), and return something {} See:
  // https://www.learncpp.com/cpp-tutorial/introduction-to-lambdas-anonymous-functions/
  std::generate(A.begin(), A.end(), [&gen, &dist]() {
    return dist(gen);
  }); // uses a lambda function
  std::generate(B.begin(), B.end(), [&gen, &dist]() {
    return dist(gen);
  }); // uses a lambda function
  ...
  std::cout <<(elapsed.count()/2.7e-06) << "\n";
...
void multiply(const std::vector<double> &m1, const std::vector<double> &m2,
  ...
  for (int ii = 0; ii < N; ++ii) {
    for (int jj = 0; jj < N; ++jj) {
      for (int s = 0; s < N; ++s) {
        m3[ii * N + jj] += m1[ii * N + s] * m2[s * N + jj];
      }
    }
  }
}
